...
- The `terraform refresh` command is used to reconcile the state Terraform knows about (via its state file) with the real-world infrastructure. This can be used to detect any drift from the last-known state, and to update the state file. First, current state of your resources with `terraform state list`. Then go to the AWS console and delete your S3 bucket `yasco-tf-test-bucket`. Display the state list again and refresh the state. Run the following commands.
...
aws_s3_bucket.tf-example-s3: Refreshing state... [id=yasco-tf-test-bucket]
...
resource "aws_s3_bucket" "tf-example-s3" {
  bucket = "yasco-tf-test-bucket-new"
  acl    = "private"
}
```
...
```bash
terraform plan -var="s3-bucket-name=yasco-new-s3-bucket-2"
```

- You can define variables with `-var` command

- Create a file name `yasco.tfvars`. Add the followings.

```bash
s3-bucket-name = "yasco-s3-bucket-newest"
```

- Run the command belov.

```bash
terraform plan --var-file="yasco.tfvars"
```
...
- Go to the AWS console and delete `tf-remote-s3-bucket-yasco` and `tf-s3-app-lock` DynamoDb table.
